...
You can install the latest version of **Pygame** 🐍 by running ```"pip install pygame"``` at the prompt. If you clone or download this repo you can change to this folder after extracting the files and run ```"python space_roids.py"```. In the future I am planning to include an ```.exe``` in the folder (Windows) that you may be able to ```double-click and run```. 🦄
